...
import (
	...
	"strike/pkg/filter/stream/auth/resource"
	"strike/pkg/protocol"
)
...
func (e *RuleEngine) invoke(headers protocol.HeaderMap) bool {
	if e.match(headers) {
		...
		if token, ok := headers.Get(e.ruleConfig.AuthConfig.TokenKey); ok {
			if payload, err := decode(e.ruleConfig.AuthConfig.PublicKey, token); err != nil {
				return false
			} else {
				headers.Set(protocol.StrikeHeaderUsername, payload["orgi"].(string))
			}
		} else {
			return false
		}

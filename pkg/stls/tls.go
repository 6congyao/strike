...
type context struct {
	...
	clusterInfo upstream.ClusterInfo
...
func NewTLSServerContextManager(config *v2.Listener, l network.Listener) (network.TLSContextManager, error) {
	mgr := &contextManager{
		contexts:  []*context{},
		listener:  l,
...
func NewTLSClientContextManager(config *v2.TLSConfig, info upstream.ClusterInfo) (network.TLSContextManager, error) {
	mgr := &contextManager{
		isClient: true,
	}
...
func (mgr *contextManager) AddContext(c *v2.TLSConfig) error {
	...
	if err != nil {
		if c.Fallback && err == ErrorGetCertificateFailed {
			fmt.Println("something wrong with certificate/key, trigger fallback")
			return nil
		}
		return err
	}

...
func normalizePath(dst, src []byte) []byte {
	...
	dst = utils.DecodeArgAppendNoPlus(dst, src)
...
func (u *URI) RequestURI() []byte {
	dst := utils.AppendQuotedPath(u.requestURI[:0], u.Path())
...
func (u *URI) Update(newURI string) {
	u.UpdateBytes(utils.S2b(newURI))
}
...
func (u *URI) updateBytes(newURI, buf []byte) []byte {
	...
	default:
		...
		buf = u.appendSchemeHost(buf[:0])
		buf = utils.AppendQuotedPath(buf, path[:n+1])
		buf = append(buf, newURI...)
		u.Parse(nil, buf)
		return buf
	}
...
func (u *URI) QueryArgs() *utils.Args {
	u.parseQueryArgs()
	return &u.queryArgs
}

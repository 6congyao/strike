...
type MockListener struct {
	net.Listener
	Mng network.TLSContextManager
}

func MockClient(t *testing.T, addr string, cltMng network.TLSContextManager) (*http.Response, error) {
...
func TestServerContextManagerWithMultipleCert(t *testing.T) {
	...
	ctxMng, err := NewTLSServerContextManager(lc, nil)
	if err != nil {
...
func TestFallback(t *testing.T) {
	...
	lc := &v2.Listener{}
	lc.FilterChains = filterChains
	serverMgr, err := NewTLSServerContextManager(lc, nil)
	if err != nil {
		t.Errorf("create context manager failed %v", err)
		return
	}
